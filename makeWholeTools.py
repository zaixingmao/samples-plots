...
def calculateSF(fileList, location0, out, verbose = False):
    files = []
    trees = []
    ...
    for fileName, location, selection, xs in fileList:
        ...
        for i in range(total):
            ...
            if trees[len(trees)-1].category == '1M1NonM':
                if passCut(trees[len(trees)-1], 'tightSS'):
                    if isData:
                        DATA_SS_TT_cat1+=1
                    else:
...
# calculateSF(makeWholeSample_cfg.sampleConfigs, makeWholeSample_cfg.preFixTools, 'veto012',True)