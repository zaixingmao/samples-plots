...
lumi = 5939.2#3990.0#2093.30#1546.91#1263.89#552.67#654.39#209.2#16.86, 578.26
Lumi = lumi #3000.0
signalScale = 5000
...
def regionSelection(tree, FS, region, lowerBound = 0, upperBound = 1):
    ...
    if FS == 'et':
        if ('signal' in region) and passBound(tree, FS, lowerBound, 'upper') and (tree.eRelIso < 0.15):
            ...
            return True
        elif ('control_anti_iso' in region) and (tree.eRelIso > 0.15):# and passBound(tree, FS, lowerBound, 'lower') and passBound(tree, FS, upperBound, 'upper'):
            return True
        else:
            return False
    if FS == 'mt':
        if ('signal' in region) and passBound(tree, FS, lowerBound, 'upper') and (tree.mRelIso < 0.15):
            ...
            return True
        elif ('control_anti_iso' in region) and (tree.mRelIso > 0.15):# and passBound(tree, FS, lowerBound, 'lower') and passBound(tree, FS, upperBound, 'upper'):
            return True
        else:
            return False
...
#         if not isData and tree.tByVLooseIsolationMVArun2v1DBnewDMwLT < 1:
#             return False
# 
#         if tree.mPt <= 30:
#             return False
# 
#         if not (30 < tree.tPt < 50):
#             return False 
        if tree.mPt <= 35:
            return False   
        if not ((40 < tree.mt_1 < 60) or (40 < tree.mt_2 < 60)):
            return False
#         if options.getWJetsSF:
#             if not (70 < tree.mt_1 < 110):
#                 return False
#         else:
#             if (40 < tree.mt_1 < 300):
#                 return False
        ...
        if options.highMETlowPZeta0NB:
            ...
            if tree.nCSVL >= 1:
                return False
        elif options.lowMET and tree.met >= 30:
                return False
...
#             if tree.nCSVL >= 1:
#                 return False
#         elif options.lowMET0NB_highDPhi:
#             if tree.met >= 30:
#                 return False
#             if tree.nCSVL >= 1:
#                 return False
    ...
    for iEntry in range(nEntries):
        ...
        if not isData:
            ...
            if (80.94 < xs < 80.96) or (136.01 < xs < 136.03):
                xs = xs*0.108*3
#             if "Z#rightarrow#tau#tau" in iCategory:
#                 xs = xs*0.8
        ...
        if regionSelection(tree, FS, "OSsignal", plots_cfg.scanRange[0], plots_cfg.scanRange[1]):
            ...
            tauPtBins = plots_cfg.tauPtBinnedWJetsL2TSF
            if regionSelection(tree, FS, "OScontrol_iso", plots_cfg.scanRange[0], plots_cfg.scanRange[1]):
                if not isData:
                    ...
                    if iCategory != 'WJets':
                        if tree.tPt < tauPtBins[0][0]:
                            histDict['WJets_OScontrol_iso_1'].Fill(value, -weight)
                        elif tree.tPt < tauPtBins[1][0]:
                            histDict['WJets_OScontrol_iso_2'].Fill(value, -weight)
                        else:
                            histDict['WJets_OScontrol_iso_3'].Fill(value, -weight)
            ...
            if regionSelection(tree, FS, "OScontrol_anti_iso", plots_cfg.scanRange[0], plots_cfg.scanRange[1]):
                if not isData:
                    ...
                    if options.getQCDFromE:
                        ...
                        if tree.tPt < tauPtBins[0][0]:
                            histDict['QCD_F_for_C_1'].Fill(value, -weight*QCD_weight)
                        elif tree.tPt < tauPtBins[1][0]:
                            histDict['QCD_F_for_C_2'].Fill(value, -weight*QCD_weight)
                        else:
                            histDict['QCD_F_for_C_3'].Fill(value, -weight*QCD_weight)
...
def buildStackFromDict(histDict, FS, option, sf):
    stack = r.THStack()
    for ikey, iColor in defaultOrder:
        ...
        if options.plotRegionC:
            ...
            if ikey in histDict.keys():
                if ikey == 'WJets':
                    histDict['WJets_OScontrol_iso'].Add(histDict['WJets_OScontrol_iso_1'])
                    histDict['WJets_OScontrol_iso'].Add(histDict['WJets_OScontrol_iso_2'])
                    histDict['WJets_OScontrol_iso'].Add(histDict['WJets_OScontrol_iso_3'])
...
def buildDelta(deltaName, histDict, bins, varName, unit, relErrMax, min, max,  WJets_L2T, fs = 'et'):
    ...
    for ikey, icolor in defaultOrder:
        ...
        if options.plotRegionC:
            ...
            if ikey in histDict.keys():
                ...
                if ikey == 'WJets':
                    tmpHist = setWJetsLooseShape(histDict['WJets_OScontrol_iso_1'], WJets_L2T[0][1], WJets_L2T[0][2], bins)
                    tmpHist2 = setWJetsLooseShape(histDict['WJets_OScontrol_iso_2'], WJets_L2T[1][1], WJets_L2T[1][2], bins)
                    tmpHist3 = setWJetsLooseShape(histDict['WJets_OScontrol_iso_3'], WJets_L2T[2][1], WJets_L2T[2][2], bins)
...
def buildHists(varName, varBins, unit, FS, option, relErrMax):
    ...
    histDict["WJets_OScontrol_iso_1"] = r.TH1F("WJets_CR_%s_%s_1" %(FS, varName), "", len(varBins)-1, varBins)
    histDict["WJets_OScontrol_iso_1"].Sumw2()
    histDict["WJets_OScontrol_iso_2"] = r.TH1F("WJets_CR_%s_%s_2" %(FS, varName), "", len(varBins)-1, varBins)
    histDict["WJets_OScontrol_iso_2"].Sumw2()
    histDict["WJets_OScontrol_iso_3"] = r.TH1F("WJets_CR_%s_%s_3" %(FS, varName), "", len(varBins)-1, varBins)
    histDict["WJets_OScontrol_iso_3"].Sumw2()
    ...
    setQCD(histDict['QCD_F_for_C_1'], qcd_scale_FtoC)
    setQCD(histDict['QCD_F_for_C_2'], qcd_scale_FtoC)
    setQCD(histDict['QCD_F_for_C_3'], qcd_scale_FtoC)
    ...
    histDict['WJets_OScontrol_iso_1'].Add(histDict['QCD_F_for_C_1'], -1.0)
    histDict['WJets_OScontrol_iso_2'].Add(histDict['QCD_F_for_C_2'], -1.0)
    histDict['WJets_OScontrol_iso_3'].Add(histDict['QCD_F_for_C_3'], -1.0)
    ...
    print "MC WJets **********************"
    wjets_scale = plots_cfg.tauPtBinnedWJetsL2TSF

    fixNegativBins(histDict['WJets_OScontrol_iso_1'])
    fixNegativBins(histDict['WJets_OScontrol_iso_2'])
    fixNegativBins(histDict['WJets_OScontrol_iso_3'])

    if "ZPrime" in histDict.keys():
        histDict["ZPrime"].Scale(signalScale)

    deltas = []
    delta, result, error, error2, histDict_withError = buildDelta('%s_delta' %(varName), histDict, varBins, varName, unit, relErrMax, 0.5, 1.5, wjets_scale, FS)
    deltas.append((delta, result, error, error2, histDict_withError))

    for iCat in histDict.keys():
        histDict[iCat].Scale(1, option)
    bkgStack = buildStackFromDict(histDict, FS, option, wjets_scale)
...
def multiPlots(FS, option):
    ...
    psfile = psfile[:psfile.rfind('.')] + '_3bins.pdf'
